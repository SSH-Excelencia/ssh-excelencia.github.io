...
document.addEventListener("DOMContentLoaded", () => {
  ...
  imgHeader.addEventListener("click", () => {
    ...
    document.getElementById("nombreIps").value = "Fernando Torres";
...
function construirTablaDocx(headers, filas) {
  const anchoCols = [55, 21, 24]; // % como en el frontend
...
function construirTablaResumenDocx() {
  ...
  const rowHeader = new TableRow({
    children: [
      celda("CRITERIO", { widthPct: 40, align: AlignmentType.CENTER, bold: true }),
      celda("CANTIDAD", { widthPct: 30, align: AlignmentType.CENTER, bold: true }),
      celda("PORCENTAJE", { widthPct: 30, align: AlignmentType.CENTER, bold: true }),
    ],
    tableHeader: true
  });
  ...
  filasResumen.forEach(f => {
    rows.push(new TableRow({
      children: [
        celda(f.criterio, { widthPct: 40, align: AlignmentType.BOTH, bold: f.criterio === "TOTAL" }),
        celda(String(f.cantidad), { widthPct: 30, align: AlignmentType.CENTER, bold: f.criterio === "TOTAL" }),
        celda(f.pct, { widthPct: 30, align: AlignmentType.CENTER, bold: f.criterio === "TOTAL" }),
      ]
    }));
  });
  ...
  sections: [
    {
      headers: {
        default: new Header({
          children: [
            new Paragraph({
              children: [new TextRun({ text: "Lista de Chequeo - Resolución 3100", bold: true })],
              alignment: AlignmentType.CENTER
            })
          ]
        })
      ...
      footers: {
        ...
        children: [
          ...
          new Paragraph({
            children: [new TextRun({ text: "Auditoria de Criterios de Habilitación", bold: true })],
            alignment: AlignmentType.CENTER
          }),
